...
class ParagraphLayoutModel(ParagraphModel, Layout):

    def __init__(self, doc_id: str, par_id: int, x1: float, y1: float, x2: float, y2: float,
                 labels: List[LabelLayoutModel]):
        ParagraphModel.__init__(self, par_id)
        Layout.__init__(self, doc_id, x1, y1, x2, y2)
        self._labels = labels

    @classmethod
    def from_data(cls, doc_id: str, par_mapping: DataFrame, labels_mapping: DataFrame):
        """
        Create a paragraph from
        Args:
            doc_id: id of the article
        ...
        for j, label_mapping in labels_mapping.iterrows():
            par_labels.append(
                LabelLayoutModel.from_data(doc_id=doc_id, mapping=label_mapping)
            )
        return cls(doc_id, par_id, par_x1, par_y1, par_x2, par_y2, par_labels)
...
#lass ParagraphMixModel(BoundingBox):
#   """

...
class ScibotLayoutLoader:
    ...
    def _load_grel(self):
        """ Loads the layouts of the g-rel files """
        ...
        for path in paths:
            ...
            for filename in glob.glob(pjoin(path, self._LABELS_EXT)):
                ...
                data = self._load_layout(filename)
                self.grel_labels[doc_id] = data

            # read paragraphs' layout
    ...
    def _load_google_nq(self):
        """ Loads the layouts of the GoggleNQ files """
    ...
    def _load_layout(path: str):
        """ Reads a file containing a layout file into a Pandas dataframe """
